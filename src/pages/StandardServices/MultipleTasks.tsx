...
function Discipline() {
    ...
    const save_asa_data = async () =>{
        if(billtasks.length == 0){
            toast.info('Select atleast one task to save.');
            return;
        }
      ...
      setLoading(true);
      const {isError, data} = await doPost(payload, 'save_asa_multiple_tasks');
      if(isError) {
          toast.error('Something went wrong with server.');
          setLoading(false);
      }else{
  ...
  return (
    <>
      <h2 className="mt-10 text-lg font-medium intro-y">Multiple Task Information</h2>
                    ...
                    >
                        ...
                        <div className="w-[20%]">
                                <FormInput 
                                    type="text"
                                    name="markup_cost"
                                    value={v.markup_cost==0?null:v.markup_cost}
                        ...
                        <div className="w-[20%]">
                                <FormInput 
                                    type="text"
                                    name="markup_cost"
                                    value={v.markup_price==0?null:v.markup_price}
                                    onChange={(e) => update_data(e, v.id, 'markup_price')}
                                    onKeyPress={(e) => {
                                        const keyCode = e.keyCode || e.which;
                                        const keyValue = String.fromCharCode(keyCode);
                                        const regex = /^[0-9\b.]+$/;
                        ...
                        <div className="w-[10%]">
                             {/* {
                                (
                                    (
                                        (v.est_hrs * v.price) + parseFloat(v.markup_price)
                                    ).toFixed(2)
                                )
                            } */}
                                    ...
                                    {/* {
                                        (() => {
                                            ...
                                            const totalPrice = (v.price * v.est_hrs) + parseFloat(v.markup_price);
...

                                    {
                                        (() => {
                                            ...
                                            if(v.markup_type == 1){
                                                var markup_tax = (v.markup_price/100);
                                                var markup_add:any = sub_total * markup_tax;
                                            } else if(v.markup_type == 2){
                                                var markup_add:any = (v.markup_price);
                                            } else {
                                                if(v.markup_price != 0){
                                                    var markup_tax = (v.markup_price/100);
                                                    var markup_add:any = sub_total * markup_tax;
