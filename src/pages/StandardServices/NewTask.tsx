...
function NewTask() {
    ...
    useEffect(() => {
        ...
        // if(came_from_client_date != 0){
        //     console.log(came_from_client_date+"i cam here")
        //     update_state_service(came_from_client_date)
        // }
        getAllDisciplines()
    }, [params]);
    ...
    const do_get_server_asa_task = async (id:any) => {
        ...
        setLoading(true);
        const {isError, data} = await doPost(payload, 'get_sepecific_task_id');
        setLoading(false);
        if(isError) {
            ...
            if (data?.action == "success") {
                ...
                setAddress(data_show.cost==0?null:data_show.cost)
                ...
                let unitePriceTotal = (data_show?.price)
                if(data_show?.tax != 0){
                ...
                const new_price = unitePriceTotal;
                setUnitPrice(unitePriceTotal)
                // console.log(new_price)
                // setUnitPrice((unitprice:any) => data_show?.total)
    ...
    const getAllDisciplines = async () => {
        ...
        setLoading(true);
        const {isError, data} = await doPost(payload, 'get_all_services');
        setLoading(false);
        if(isError) {
            toast.error('Something went wrong with server.');
            setLoading(false);
    ...
    const get_standard_services = async (slugFromUrl:any) => {
      const payload = {
        ...
        //console.log(payload);
      setLoading(true);
      const {isError, data} = await doPost(payload, 'get_specific_services');
      if(isError) {
          toast.error('Something went wrong with server.');
          setLoading(false);
      }else{
          if (data.action == "success") {
            ...
            setMarkup(data_show?.markup==1?true:false)
            setPricingMode(data_show?.markup_type==0?"":data_show?.markup_type)
            setNameInfo(data_show?.markup_price==0?null:data_show?.markup_price)
    ...
    const save_service = async (vid:any = 0, cid:any = 0) =>{
        ...
        // return;
        setLoading(true);
        const {isError, data} = await doPost(payload, client_number==0?'add_new_services':'add_asa_task');
        if(isError) {
            toast.error('Something went wrong with server.');
            setLoading(false);
        }else{
    ...
    const update_state_service = async (id:any) => {
        ...
        if(isError) {
            ...
            if (data.action == "success") {
                ...
                setContactEmails(data_show.est_hrs)
                setPoNumber(data_show.price)
                let unitePriceTotal = (data_show.est_hrs)
                if(data.data_show?.tax != 0){
    ...
    const calculate_unit_price =  async (val:any, fo:any) => {
            ...
            const markup__ = fo=="markup"?val:nameinfo==null?0:nameinfo;
            const tax_ = fo=="tax"?val:tax==null?0:tax;
            let TaxPrice = 0;
            ...
            const plus_all_sub = ((total_unite) + parseFloat(new_markup));

            if(tax_!=0){
                const taxRate = (tax_/100); // Assuming tax rate of 10%
                const totalPrice = plus_all_sub;
                const taxcount = totalPrice * taxRate;
                TaxPrice = (taxcount);
            }
            const plus_all = (plus_all_sub + TaxPrice);
    ...
    return (
      <>
        ...
        <div className="flex flex-col items-center mt-8 intro-y sm:flex-row  pl-2">
          <h2 className="mr-auto text-lg font-medium">{slugFromUrl==''?"New":"Update"} Task Information</h2>
        </div>
        ...
        <div className="pb-3">
            ...
            <div className={"col-span-12 text-left"}>
                <label className={"pb-2 text-sm flex justify-between align-center"}>
                    <div>Service Name  <span className="redclass">*</span></div>
                    {
            ...
            <div className="col-span-12 flex gap-5">
            {
                ...
                <div className={"col-span-4 text-left"}>
                    <label className={"pb-2 text-sm"}>Cost</label>
                    <FormInput type="text"
                                name={"address"}
                                value={address}
                ...
                <div className={"col-span-4 text-left"}>
                    <label className={"pb-2 text-sm"}>Tax (%)</label>
                    <FormInput type="text"
                                name={"tax"}
                                value={tax}
                                onChange={(e)=>
                                    {
                                        ...
                                        calculate_unit_price(e.target.value, 'tax')
                                    }
                                }
                                onKeyPress={(e) => {
                                    const keyCode = e.keyCode || e.which;
            ...
            <div className={"col-span-12 text-left"}>
             ...
             <div className={"col-span-12 text-left"}>
                <label className={"pb-2 text-sm"}>Internal Task notes</label>
                <FormTextarea 
                            name={"pdfdescription"}
