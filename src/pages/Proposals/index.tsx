...
function Proposals() {
    ...
    const getAllDisciplines = async () => {
      ...
      setLoading(true);
      const {isError, data} = await doPost(payload, 'get_all_templates');
      setLoading(false);
      if(isError) {
    ...
    const save_department = async () =>{
      ...
      setLoading(true);
      const {isError, data} = await doPost(payload, 'add_new_proposal');
      if(isError) {
          toast.error('Something went wrong with server.');
          setLoading(false);
      }else{
    ...
    const update_discipline = async () =>{
      ...
      setLoading(true);
      const {isError, data} = await doPost(payload, 'update_template');
      if(isError) {
          toast.error('Something went wrong with server.');
          setLoading(false);
      }else{
          if (data.action == "success") {
            ...
            resetStates();
            setAddDepartmentModal(false)
            toast.success("Proposal information updated successfully!")
            getAllDisciplines()
          }
          else {
              setLoading(false);
    ...
    const delete_department = async () =>{
      ...
      setLoading(true);
      const {isError, data} = await doPost(payload, 'delete_template');
      setLoading(false);
      if(isError) {
          toast.error('Something went wrong with server.');
          setLoading(false);
    ...
    const get_discipline_update = async (val:any) => {
      ...
      setLoading(true);
      const {isError, data} = await doPost(payload, 'get_specific_template');
      if(isError) {
          toast.error('Something went wrong with server.');
          setLoading(false);
      }else{
          if (data.action == "success") {
            ...
            setEditorData(data?.data.description==null?'':data?.data.description);
  ...
  return (
    <>
      <h2 className="mt-10 text-lg font-medium intro-y">Proposal Templates</h2>
        ...
        <Dialog.Panel>
          <div className="p-5 text-center">
            ...
            <div className="mt-2 text-slate-500">
              Do you really want to delete this Template? <br />
            ...
            <Dialog.Panel>
                <Dialog.Title>
                    <h2 className="mr-auto text-base font-medium">
                        {editid!=null?"Update Template":"Add New Template"}
                    </h2>
                </Dialog.Title>
                <Dialog.Description className="grid grid-cols-12 gap-4 gap-y-3">
                    <div className="col-span-12">
                        <FormLabel htmlFor="modal-form-1">
                          Template Name <span className="redclass">*</span>
                        </FormLabel>
                        <FormInput 
                          id="modal-form-1" 
                          type="text" 
