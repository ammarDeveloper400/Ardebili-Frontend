...
export default function ViewPDF() {
    ...
    const get_standard_services = async () => {
        const payload = {
          ...
          //   return;
        setLoading(true);
        const {isError, data} = await doPost(payload, 'get_react_asa_pdf_data');
        if(isError) {
            ...
            if (data.action == "success") {
              setLoading(false);
              setAsa(data?.asa);
              setrReciied(data?.asa.recipients)
              setASAEMAILS(data?.asa.client_emails);
      ...
      const download_pdf = async () => {
        ...
        const payload = {
            ...
            //   return;
          setLoading(true);
          const {isError, data} = await doPost(payload, 'do_create_download_pdf');
          if(isError) {
              toast.error('Something went wrong with server.');
              setLoading(false);
          }else{
      ...
      const send_to_client = async (ros:any) => {
        const payload = {
            token:getLoggedObject()?.token,
            asa_id: ros
          }
        ...
        //return;
          setLoading(true);
          const {isError, data} = await doPost(payload, 'do_issue_asa_to_client');
          if(isError) {
              toast.error('Something went wrong with server.');
              setLoading(false);
          }else{
      ...
      const handleContextMenu = (event) => {
        ...
        return (
            ...
            <div className="p-5 custom-p5">
                ...
                <div className=" flex custom_pdf_flex" >
                ...
                <div className="htmlContract px-0 custom-60" ref={printRef}>
                    ...
                    {/* {pdfdata != null &&
                        <object data={'data:application/pdf;base64, '+pdfdata+"#toolbar=0"} title="Your Toolbar Title" type="application/pdf" width="100%" height="100%" ></object>
                    } */}
                    {pdfdata != null &&
            ...
            <div className="flex w-[100%] justify-end gap-3 pr-11">
        ...
        >
...

                {
                    ...
                    <Button variant="primary" type="button" className="w-20  w-[120px]" ref={sendButtonRef} onClick={()=>send_to_client(asa.req_id)}>
