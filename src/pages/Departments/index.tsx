...
function Departments() {
    ...
    const save_department = async () =>{
      ...
      setLoading(true);
      const {isError, data} = await doPost(payload, 'add_new_department');
      if(isError) {
          toast.error('Something went wrong with server.');
          setLoading(false);
      }else{
    ...
    const delete_department = async () =>{
      ...
      setLoading(true);
      const {isError, data} = await doPost(payload, 'delete_department');
      setLoading(false);
      if(isError) {
          toast.error('Something went wrong with server.');
          setLoading(false);
      ...
      if (departments) {
          if (q){
              ...
              let results = departments.filter((item) => {
                  return item.role !== null && item.role.toLowerCase().includes(q.toLowerCase());
              })
  ...
  return (
    <>
      <h2 className="mt-10 text-lg font-medium intro-y">All Departmets</h2>
        ...
        <Dialog staticBackdrop  open={adddepertmentmodal} 
            ...
            <Dialog.Panel>
                ...
                <Dialog.Description className="grid grid-cols-12 gap-4 gap-y-3">
                    <div className="col-span-12">
                        <FormLabel htmlFor="modal-form-1">
                            Department Name <span className="redclass">*</span>
                        </FormLabel>
                        <FormInput 
                          id="modal-form-1" 
                          type="text" 
                ...
                <Dialog.Footer>
                    ...
                    <Button variant="primary" type="button" className="w-20" ref={sendButtonRef} onClick={()=>save_department()}>
                        Add
                    </Button>
                </Dialog.Footer>
            </Dialog.Panel>
    ...
    // return (
    //   <>
    //     <h2 className="mt-10 text-lg font-medium intro-y">All Departments</h2>
    ...
    //                     <DataTable
    //                         columns={columns}
    //                         data={filtered ? filtered : departments}
