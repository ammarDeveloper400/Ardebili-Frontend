...
function SendClient() {
    ...
    if(params.state?.version == 0){
        version_number = 0;
    } 
    const version__ = version_number;
    ...
    // if(type_quote == 1){
    //     var number_format = "P"+new Date().getFullYear();
    // } else {
    //     var number_format = "A"+new Date().getFullYear();
    // }

    const {getLoggedObject, setLoggedObject, isLoggedIn, checkLogin} = useAuth();
    const [proposalnumber, setProposalNumber] = useState<any | null>(slugFromUrl)
    ...
    const [servicedescription, setServiceDescription] = useState<any | null>('')
    ...
    const get_standard_services = async (slugFromUrl:any) => {
      ...
      if(isError) {
          ...
          if (data.action == "success") {
            setLoading(false);
            if(data?.asa_data != null){
                const data_show = data?.asa_data;
                console.log(data_show);
                setAsa(data_show);
                setProposalNumber(data_show.quote_number)
                setRequestDate(data_show.asa_request_date)
                setProjectName(data_show.projectname)
                setProjectNumber(data_show.projectnumber)
                setClientprojectnumber(data_show.clientprojectnumber)
                setClient(data_show.client)
                setClientContact(data_show.clientcontact)
                setContactEmails(data_show.contactemails)
                setServiceDescription(data_show.service_description)
                setUrgencyWork(data_show.asa_urgency_work)
                ...
                setPdfDescription(data_show.description==null?"":data_show.description)
                ...
                if(data_show.tasks_array != null){
                    setBillTasks(data_show.task_completed)
                }
                if(data_show.costs_array != null){
                    setBillCosts(data_show.cost_completed)
                }
                //setClientAddress(data_show?.client_address)
            }
    ...
    const save_client_data = async (type:any = 0, revise:any = 0) =>{
        if(loading) return;
        if(type===2){} else {
            ...
            if(allcosts.length > 0 && billcosts.length == 0){
        ...
        if(isError) {
            ...
            if (data.action == "success") {
                ...
                if(type==2){
                    ...
                    toast.success("ASA information saved!")
                    navigate('/quote/pdf/'+slugFromUrl)
                } else {
                    navigate('/quote/pdf/'+slugFromUrl)
                }
            }
      ...
      <>
        ...
        <div className="flex flex-col items-center mt-8 intro-y sm:flex-row  pl-2">
            <h2 className="mr-auto text-lg font-medium">
                {quote_type===1?"New Proposal":"Additional Service Estimate"}
            </h2>
        </div>
        ...
        {quote_type!=1 &&
            <div className="mt-5  pl-2 ">
                ...
                {/* {proposalnumber} */}
                {proposalnumber!=null?proposalnumber+(asa && asa?.revision_number!=0?"."+asa?.revision_number:""):slugFromUrl}
                </div>
            </div>
        }
        <div className="pb-3">
            ...
            <div className={"col-span-6 text-left"}>
                <label className={"pb-2 text-sm"}>{quote_type===1?"Proposal":"Project"} Number  <span className="redclass">*</span></label>
                {quote_type===1 ?
                    <div className="font-bold text-2xl">
                       {proposalnumber!=null?proposalnumber+(asa && asa?.revision_number!=0?"."+asa?.revision_number:""):slugFromUrl}
                    </div>
            ...
            {/* <div className={"col-span-12 text-left"}>
                                    <label className={"pb-2 text-sm"}>Client Address <span className="redclass">*</span></label>
                                    <FormInput type="text"
                                                name={"clientaddress"}
            ...
            <div className={"col-span-12 text-left"}>
                <label className={"pb-2 text-sm"}>Description  <span className="redclass">*</span></label>
                <ClassicEditor
            ...
            <div className="flex  mb-5 align-center">
                <Button  className="mr-2shadow-md text-primary w-[120px]"
                        onClick={(event: React.MouseEvent)=> {
                            ...
                            save_client_data(2, -1)
                            navigate("/new/task/0", { state: { asa_number: slugFromUrl, proposal:quote_type, revise: version__ } })
                        }}
                >
                    New Task
                </Button>
                ...
                onClick={(event: React.MouseEvent)=> {
                    event.preventDefault();
                    save_client_data(2, -1)
                    navigate("/multiple/tasks", { state: { asa_number: slugFromUrl, proposal:quote_type, revise: version__} })
                }}
                ...
                {(provided:any) => (
                    ...
                    {alltasks.map((v:any, id:any) => (
                        ...
                        <div className="w-[5%]">
                            ...
                            type="checkbox"
                            checked={billtasks && billtasks.includes(v.id as never)}
                                ...
                                onClick={
                                    ()=> 
                                    {
                                        if(v.single_price == 0){} 
                                            ...
                                            save_client_data(2)
                                            navigate("/new/task/0", { state: { asa_number: slugFromUrl, specific_task: v.id, edit_mode: 1, proposal:quote_type,} })
                                        }
                                    }
                                }
                                >{v.task_name}{v.label!=null?" - "+v.label:""}</span>
                        ...
                        <div className="w-[12%]">
                            <FormInput
                                ...
                                maxLength={4}
                                // onChange={(e)=>HandleTaskUpdate(e.target.value, 'task_time', id)}
                                onChange={(e)=>
                                    {
                        ...
                        <div className="w-[12%]">
                            <FormInput
                                ...
                                disabled={pricingmode==2 || pricingmode==0}
                                // onChange={(e)=>HandleTaskUpdate(e.target.value, 'price', id)}
                                onChange={(e)=>
                        ...
                        <div className="w-[12%]">
                            <FormInput
                                ...
                                defaultValue={v.cost}
                                disabled={pricingmode==2 || pricingmode==0}
                                // onChange={(e)=>HandleTaskUpdate(e.target.value, 'cost', id)}
                                onChange={(e)=>
                                    {
                                        const valuee = e.target.value || 0;
            ...
            <div className="flex  mb-5 align-center">
                <Button  className="mr-2shadow-md text-primary w-[120px]"
                        onClick={(event: React.MouseEvent)=> {
                            ...
                            navigate("/new/cost/0", { state: { asa_number: slugFromUrl, proposal:quote_type, revise: version__ } })
                        }}
                >
                    New Cost
                </Button>
                ...
                onClick={(event: React.MouseEvent)=> {
                    event.preventDefault();
                    save_client_data(2, -1)
                    navigate("/multiple/cost", { state: { asa_number: slugFromUrl, proposal:quote_type , revise: version__} })
                }}
                className="blue_color ml-5">
                    + Add Multiple Costs
                </div>
                ...
                {(provided:any) => (
                    ...
                    {allcosts.map((v:any, id:any) => (
                        ...
                        <div className="w-[5%]">
                            ...
                            className="flex-none border-slate-400 checked:border-primary"
                            type="checkbox"
                            checked={billcosts && billcosts.includes(v.id as never)}
                            onChange={() => {do_update_costs_checkbox(v.id)}}
                            />
                        </div>
                            <div className="w-[32%]">
                                ...
                                onClick={
                                    ()=> 
                                    {
                                        if(v.single_price == 0){} 
                                            ...
                                            save_client_data(2)
                                            navigate("/new/cost/0", { state: { asa_number: slugFromUrl, specific_task: v.id, edit_mode: 1, proposal:quote_type,} })
                                        }
                                    }
                                }
